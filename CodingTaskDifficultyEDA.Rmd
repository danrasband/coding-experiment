...
```{r results="asis"}
model <- lm(t1t2_mins_diff ~ control_t1_id_1 + control_t2_id_3 + berkeley_student + upwork + age + years_experience, data=both_attempted)
stargazer(model)
```

```{r results="asis"}
model <- lm(t1t2_score_diff ~ control_t1_id_1 + control_t2_id_3 + berkeley_student + upwork + age + years_experience, data=both_attempted)
stargazer(model)
```
...
stargazer(between.subjects.mins.lm)
```
...
stargazer(between.subjects.score.all.lm)
```
...
stargazer(between.subjects.mins.all.lm)
```
